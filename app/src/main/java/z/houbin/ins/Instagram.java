...
public class Instagram extends BaseModule {
    ...
    public void download() {
        ...
        if (!TextUtils.isEmpty(mInfo.video)) {
            onLoadStart(DownloadManager.getImpl().startDownload(mInfo.video, ".mp4"));
        }
        if (mInfo.image.size() != 0) {
            for (int i = 0; i < mInfo.image.size(); i++) {
                onLoadStart(DownloadManager.getImpl().startDownload(mInfo.image.get(i), ".jpg"));
            }
        }
    }
    ...
    public void download(String tag) {
        super.download(tag);
        if (!TextUtils.isEmpty(mInfo.video)) {
            onLoadStart(DownloadManager.getImpl().startDownload(mInfo.video, ".mp4", tag));
        }
        if (mInfo.image.size() != 0) {
            for (int i = 0; i < mInfo.image.size(); i++) {
                onLoadStart(DownloadManager.getImpl().startDownload(mInfo.image.get(i), ".jpg", tag));
            }
        }
    }
}
